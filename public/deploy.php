...
$commands = [
    ...
    'echo "📦 Instalando dependencias..." && cd ' . dirname(__FILE__) . '/../ && composer install --no-dev --optimize-autoloader 2>&1',
    'echo "🔑 Generando clave de aplicación..." && cd ' . dirname(__FILE__) . '/../ && php artisan key:generate --force 2>&1',
    'echo "⚡ Limpiando caché..." && cd ' . dirname(__FILE__) . '/../ && php artisan config:cache 2>&1',
    'echo "📋 Listando rutas de webhook..." && cd ' . dirname(__FILE__) . '/../ && php artisan route:list --path=webhooks 2>&1',
];
